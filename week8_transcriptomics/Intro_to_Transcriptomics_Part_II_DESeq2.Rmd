...
rna_counts <- read.csv("C:/Users/zhang/OneDrive/college/qbio490/qbio_490_lucia/analysis_data/brca_rna_count_data.csv")
rna_genes <- read.csv("C:/Users/zhang/OneDrive/college/qbio490/qbio_490_lucia/analysis_data/brca_rna_gene_data.csv")
rna_clinical <- read.csv("C:/Users/zhang/OneDrive/college/qbio490/qbio_490_lucia/analysis_data/brca_rna_clinical_data.csv")
up_reg_results <- read.csv("C:/Users/zhang/OneDrive/college/qbio490/qbio_490_lucia/analysis_data/up.csv", row.names=1, header=TRUE)
down_reg_results <- read.csv("C:/Users/zhang/OneDrive/college/qbio490/qbio_490_lucia/analysis_data/down.csv", row.names=1, header=TRUE)
```
...
row_sums <- rowSums(rna_counts[, c(-1,-2)], na.rm=TRUE)
# row_sums
...
```{r}
# write code here
# windows wouldn't let me save my dataframe so wade sent me an up regulated and down regulated csv
```
...
Fibroblast Growth Factor 4. They are broadly involvd in cell survivle like embryonic development, cell growth, tissue repair, and so on. One key function is tumor growth and invasion. It has oncogenic transforming activity. 
...
More highly expressed in young or old patients:
Google the gene. What is its full name, and what appears to be its function? Genecards generally has a pretty good summary:
RNA U1 Small Nuclear 52 Pseudogene is a pseudogene. This means that it resembles a gene but has been mutated into an inactive form, so it doesn't function. Perhaps it is closely linked to or is involved in the regulation of a non-pseudogene.

Write the results, up_reg_results, and down_reg_results df's to analysis_data using write.csv().

```{r}
write.csv(up_reg_results, "up.csv", row.names=FALSE)
write.csv(down_reg_results, "down.csv", row.names=FALSE)
```
...
top-right genes: up, sig
bottom-right genes: up, not sig
top-left genes: down, sig
bottom-left genes: down, not sig
top-middle genes: neither, neither
bottom-middle genes: neither, neither
...
# create plot here
par(mar=c(1,1,1,1))
res <- rbind(up_reg_results, down_reg_results)
volcano <- EnhancedVolcano(res, lab = rownames(res), x="log2foldchange", y="pvalue")
volcano
# i didn't have the original results dataframe w/o everything filtered because deseq wouldn't run on my computer so wade had to send it to me
```
...
```{r}
# create plot here
# i didn't do this part because my plot was so weird because i didn't have the results dataframe and the only data i had all had very similar logp values
```
