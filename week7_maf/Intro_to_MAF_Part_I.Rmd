...
P728A is the amino acid change in the protein sequence and the position it's in. A represents the amino acid that has replaced P. fs means frameshift mutation, and 11 indicates the stop codon occurs 11 amino acids downstream of the mutated position. 
...
This tells us the patients who have both mutations.
...
```{r Exercise 8.1}
jpeg("C:/Users/zhang/OneDrive/college/qbio490/qbio_490_lucia/week7_maf/oncoplot_10.png")

oncoplot_10 <- oncoplot(maf = maf_object,
         top = 10,
         borderCol = NA) 
...
```{r Exercise 9.1}
mask <- ifelse(maf_object@clinical.data$age_category == "Young", T, F)
...
```{r Exercise 9.3}
mask <- ifelse(maf_object@clinical.data$age_category == "Old", T, F)
old_patient_barcodes <-maf_object@clinical.data$Tumor_Sample_Barcode[mask]
...
```{r Exercise 9.4}
jpeg("C:/Users/zhang/OneDrive/college/qbio490/qbio_490_lucia/week7_maf/cooncoplot.png")
...
Hypothesized Reason: GATA encodes for transcription factors which regulate the expression of genes. It might be because, in young people, it may disrupt cellular processes. 
...
```{r Exercise 10.1}
jpeg("C:/Users/zhang/OneDrive/college/qbio490/qbio_490_lucia/week7_maf/muc16_lollipop.png")
...
TP53 does not have a relationship to survival because the p-value is 0.203, which is higher than 0.05 and thsu not significant. TP53 regulates p53, a tumor suppressor gene that regulates the formation and growth of cancer cells.

What can you conclude from your plot?
There is no significance. 

Is this what you would expect according to the literature? Why or why not?
Ii wouldn't expect this considering what a large role TP53 plays.
